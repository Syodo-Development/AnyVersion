...
public class CreativeContentHandler extends PacketHandler<CreativeContentPacket> {
    ...
    @Override
    public void handle(ProtocolVersion version, CreativeContentPacket packet) {
        List<CreativeItemData> c = new ArrayList<>(packet.getContents());
        packet.getContents().clear();
        for(CreativeItemData data : c) {
            if(Registries.ITEM.getProtocolVersion(ItemDefinition.of(data.getItem().getDefinition().getIdentifier())).protocol() <= version.protocol()) {
